...
const templatePath = path.join(repoRoot, "problem", "data", "附件5", "result3.xlsx");
const payloadPath = path.join(repoRoot, "outputs", "q3_multistage", `payload_stages${tag}_K${K}.json`);
const outputDir = path.join(repoRoot, "outputs", "q3_multistage");
...
const writePriceSheets = (sheet, kwhKey, totalKey, costKey, tableName) => {
...
writePriceSheets(planSheet, "plan_kwh", "plan_total_kwh", "plan_cost_yuan");
writePriceSheets(adjustSheet, "adjusted_kwh", "adjusted_total_kwh", "adjusted_cost_yuan");

// 充放电量：官方模板仅给出 4 个示例日，扩展为 334 天 × 6 段 = 2005 行。
...
const storageTable = storageSheet.tables.add("A1:F2005", true, "Q3StorageTable");
