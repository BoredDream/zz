...
def fig2a() -> dict:
    ...
    with matplotlib.rc_context(RC):
        ...
        axp.step(step, np.r_[price, price[-1]], where="post", color=C_ACCENT, lw=1.5)
        ...
        axp.set_title(f"{tgt}　计划购电量、实际净负荷与电价", pad=5)
        if peak.any():
            axp.annotate("尖峰价时段 ≥1.25 元/kWh", xy=(t[peak][0] + 1.5, 1.70),
                         fontsize=7.6, color=C_ACCENT, va="top")

        ax.fill_between(step, 0, plan_s, step="post", color=C_MAIN, alpha=0.18,
        ...
        ax.step(step, np.r_[net, net[-1]], where="post", color=C_ACT, lw=1.4,
                ls=LS_ALT, zorder=5, label="实际净负荷")
        ax.step(step, np.r_[fc, fc[-1]], where="post", color=C_GREY, lw=1.0,
                ls=LS_ALT2, zorder=4, label="模型预测净负荷")
        ax.axhline(0, color=C_GREY_L, lw=0.6, zorder=2)

        lo = int(np.argmin(net))
        ax.plot([t[lo]], [net[lo]], marker=MK_ALT2, ms=4.2, mfc="white",
                mec=C_ACT, mew=1.0, zorder=7)
        box(ax, f"光伏反超负载，最低 {net[lo]:.0f}", (t[lo], net[lo]), (6, -18), C_TEXT)
        peak_plan = float(plan[peak].sum()) if peak.any() else 0.0
        ax.annotate(f"尖峰价时段计划购电仅 {peak_plan:,.0f} kWh", xy=(20.9, 810),
                    xytext=(15.0, 1080), textcoords="data", fontsize=8,
                    color=C_ACCENT, ha="center", va="center", zorder=9,
                    arrowprops=dict(arrowstyle="-", lw=0.6, color=C_GREY, shrinkA=2,
                                    shrinkB=2, connectionstyle="arc3,rad=0.18"),
    ...
    return {"files": out, "meta": {"peak_plan": peak_plan,
                                   "plan_total": float(plan.sum())}}
